...
export async function getBudgetItemReferences(
	...
	const scopedIdentities = indexes.length
		...
		: [];
	const foundByOperationalId = new Set(items.map((item) => item.id));
	if (!version) {
		return {
			found: [],
			missing: ids.filter((id) => !foundByOperationalId.has(id)),
		};
	}
	...
	const missing = ids.filter(
		(id) => !foundByOperationalId.has(id) && !foundVersionItemIds.has(id),
	);
	const versionItemById = new Map(versionItems.map((item) => [item.id, item]));
	...
	const found: BudgetItemReferenceRow[] = [];
	for (const id of ids) {
	...
	for (const item of items) {
		...
		const versionItem = versionItemByIdentity.get(identity.id);
		if (!versionItem) continue;
		found.push({
...
export async function getBalanceRows(
	...
	const db = tx ?? prisma;
	const identityIds = references.map((ref) => ref.identityId);
	...
	for (const row of ledgerRows) {
		...
		if (row.eventType === "COMMITMENT_INCREASE") {
			...
			if (row.sourceType === "CONTRACT_MEASUREMENT") {
				...
				current.independentConsumed = current.independentConsumed.plus(amount);
			}
		} else if (row.eventType === "INCURRED_REVERSAL") {
			if (row.sourceType === "CONTRACT_MEASUREMENT") {
				current.contractConsumed = current.contractConsumed.minus(amount);
			} else {
				current.independentConsumed = current.independentConsumed.minus(amount);
			}
		}
	...
	for (const row of pendingRows) {
		pendingByIdentity.set(
			...
			(pendingByIdentity.get(row.budgetItemIdentityId) ?? new Decimal(0)).plus(
				row.amount,
			),
		);
	}
	...
	return references.map((ref) => {
		...
		return {
			...
			pendingImpact: pendingByIdentity.get(ref.identityId) ?? new Decimal(0),
		};
	});
}
